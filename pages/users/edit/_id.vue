        ...
        <v-card class="pa-5" outlined tile>
          ...
          <v-form @submit.prevent="SubmitForm" v-model="valid" pa-3 lazy-validation>
            ...
            <v-row>
              ...
              <v-col cols="3">
                <v-text-field v-model="password" :counter="150" :label="$t('users.table.password')" required
                  :placeholder="$t('users.table.password')" outlined dense></v-text-field>
              </v-col>
              <v-col cols="3">
                <v-text-field v-model="password_confirmation" :counter="150"
              ...
              <v-col cols="3">
                <!-- <v-text-field v-model="zone" :counter="150" :rules="nameRules" :label="$t('users.table.zone')" required
                  :placeholder="$t('users.table.zone')" outlined dense></v-text-field> -->
...
export default {
  ...
  methods: {
    ...
    async fetch() {
      let res = await this.$axios.get("/auth/edit/" + this.getId);
      console.log('resUser', res);
      const user = res.data.user;
      this.departmen_id = user.departmen_id;
      ...
      let userType = await this.$axios.get("/typeuser/edit/" + user.type_user);
      this.type_user_selected = userType.data.type_user.type_name;

      let userDepartmen = await this.$axios.get("/department/edit/" + user.departmen_id);
      const department = userDepartmen.data.department;
      this.departmen_selected = department.dep_name

      let province = await this.$axios.get("/setting/province/" + user.province);
      let district = await this.$axios.get("/setting/province/district/" + province.data.province[0].pid);
      this.cbx_district = district.data.district;
      let district_seleted = await this.$axios.get("/setting/district/" + user.district);
      let village = await this.$axios.get("/setting/district/village/" + district_seleted.data.district[0].did);
      console.log('village', village.data.villages);
      this.cbx_village = village.data.villages;
    ...
    async SubmitForm() {
      try {
        ...
        const res = await this.$axios.post('auth/resolve', formData)
        console.log(res);
        if (res.data.success) {
          this.dialog = true;
          // this.dep_name = '';
