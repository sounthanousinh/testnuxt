              ...
              <thead class="teal accent-3">
                <tr>
                  <th width="15" class="text-left">
                    {{ $t('typecontact.table.no') }}
                  </th>
                  <th class="text-left">
                    {{ $t('typecontact.table.type_name') }}
                  </th>
                  <th class="text-left">
                    {{ $t('typecontact.table.type_status') }}
                  </th>
...
export default {
  ...
  methods: {
    async fetch() {
      let res = await this.$axios.get("/contact/show");
      console.log('res', res);
    ...
    create() {
      this.$router.push('/' + this.$i18n.locale + '/type-contact/create')
    },

    edit(id) {
      this.$router.push('/' + this.$i18n.locale + '/type-contact/edit/' + id)
    },
    ...
    async confirm_deleted() {
      let res = await this.$axios.get("/contact/delete/" + this.depId);
      console.log('res', res);
    ...
    async handlePagination(pageId) {
      // console.log(pageId);
      let res = await this.$axios.get("/contact/show?page=" + pageId);
      console.log('res', res);
