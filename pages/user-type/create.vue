            ...
            <v-row>
              ...
              <v-col cols="6">
                <v-text-field v-model="type_name" :counter="150" :rules="nameRules"
                  :label="$t('usertypes.table.type_name')" required :placeholder="$t('usertypes.table.type_name')"
  ...
  methods: {
    async SubmitForm() {
      try {
        ...
        const res = await this.$axios.post('typeuser/store', formData)
        console.log(res);
        if (res.data.success) {
          this.dialog = true;
