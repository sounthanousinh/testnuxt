            ...
            <template v-slot:default>
              <thead class="teal accent-3">
                <tr>
                  ...
                  <th class="text-left">
                    {{ $t('issues.table.issue_name') }}
                  </th>
                  <th class="text-left">
                    {{ $t('issues.table.type_status') }}
                  </th>
              ...
              <tbody>
                <tr v-for="(item, index) in desserts" :key="item.name">
                  ...
                  <td>{{ item.issue_status == 1 ? 'Active' : 'InActive' }}</td>
                  <td>
...
export default {
  ...
  methods: {
    async fetch() {
      let res = await this.$axios.get("/issue/show");
      console.log('res', res);
    ...
    create() {
      this.$router.push('/' + this.$i18n.locale + '/issue/create')
    },

    edit(id) {
      this.$router.push('/' + this.$i18n.locale + '/issue/edit/' + id)
    },
    ...
    async confirm_deleted() {
      let res = await this.$axios.get("/issue/delete/" + this.depId);
      console.log('res', res);
