            ...
            <v-row>
              <v-col cols="4">
                <v-combobox v-model="title_issue_name" :items="header_issue" item-text="issue_name" item-value="id"
                  :label="$t('subissues.table.title_issue_id')" @change="onChangeDepartment" outlined dense></v-combobox>
              </v-col>
              <v-col cols="4">
  ...
  methods: {
    ...
    async SubmitForm() {
      try {
        ...
        const res = await this.$axios.post('subtitleissue/store', formData)
        console.log(res);
        if (res.data.success) {
          this.dialog = true;
          // this.title_issue_id = '';
