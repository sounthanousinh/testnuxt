              ...
              <thead class="teal accent-3">
                <tr>
                  <th width="15" class="text-left">
                    {{ $t('subissues.table.no') }}
                  </th>
                  <th class="text-left">
                    {{ $t('subissues.table.title_issue_id') }}
                  </th>
                  <th class="text-left">
                    {{ $t('subissues.table.sub_name') }}
                  </th>
                  <th class="text-left">
                    {{ $t('subissues.table.sub_status') }}
                  </th>
                  <th width="280" class="text-center"> </th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(item, index) in desserts" :key="index">
                  <td>{{ (index + 1) }}</td>
                  <td>{{ item.title_issued.issue_name }}</td>
                  <td>{{ item.sub_name }}</td>
                  <td>{{ item.sub_status == 1 ? 'Active' : 'InActive' }}</td>
                  <td>
...
export default {
  ...
  methods: {
    async fetch() {
      let res = await this.$axios.get("/subtitleissue/show");
      console.log('res', res);
    ...
    create() {
      this.$router.push('/' + this.$i18n.locale + '/sub-issue/create')
    },

    edit(id) {
      this.$router.push('/' + this.$i18n.locale + '/sub-issue/edit/' + id)
    },
    ...
    async confirm_deleted() {
      let res = await this.$axios.get("/subtitleissue/delete/" + this.depId);
      console.log('res', res);
